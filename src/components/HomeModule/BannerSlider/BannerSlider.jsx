...
const BannerSlider = ({ dots, images,aspectRatio,styleObj}) => {
  var settings = {
    dots: false,
    infinite: true,
    speed: 1000,
  ...
  console.log('Images:', images,aspectRatio,styleObj);
      ...
      <Slider {...settings}>
        {images?.map((item,index) => (
          <div id={index+1} key={index} >
            <img style={{ aspectRatio: aspectRatio, width: '100%' }} src={item} alt="" />
          </div>
        ))}
